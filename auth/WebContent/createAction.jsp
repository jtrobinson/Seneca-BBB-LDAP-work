...
<%
	...
	if (mPwd == null || mPwdre == null || vPwd == null
			|| vPwdre == null || meetingName == null) {
	...
	if (mPwd.length() == 0 || mPwdre.length() == 0
			|| vPwd.length() == 0 || vPwdre.length() == 0
	...
	} else if (!mPwd.equals(mPwdre)) { // checking for passwords to match
		session.setAttribute("fail", "2");
		if (check == null)
			response.sendRedirect("create.jsp");
		else
			response.sendRedirect("create.jsp?isLecture=true");
	} else if (!vPwd.equals(vPwdre)) { // checking for viewer passwords to match
		session.setAttribute("fail", "3");
		if (check == null)
			response.sendRedirect("create.jsp");
		else
			response.sendRedirect("create.jsp?isLecture=true");
		//making sure there is no invalid characters in both section and meeting Name
	} else if (StringUtils.indexOfAny(meetingName, "~") != -1 || StringUtils.indexOfAny(section, "~") != -1
		...
		String dataString = compressMeeting(meetingName, mPwd, vPwd,
				allowGuests, recordable);
		//	out.print("DEBUG: Data string is: " + dataString + "<br >");

		saveMeeting(ldap.getUserID(), meetingName, mPwd, vPwd,
				allowGuests, recordable);
